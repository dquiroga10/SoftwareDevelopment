...
public class Controller {
    ...
    public void switchFromTitleToGenerating(int skillLevel) {
        ...
        currentState.setSkillLevel(this.skillLevel);
     ...
     */
    public void switchFromPlayingToWinning(int pathLength, float batLev) {
    	if(driver != null) {
	    	pathLength = robot.getOdometerReading();
	    	batLev = driver.getEnergyConsumption();
    	}
    	//System.out.println("path: "+ pathLength);
    	robot.resetOdometer();
    	robot.setBatteryLevel(3000);//TODO set back to 3000
        currentState = states[3]; // ORIGINAL LINE
        currentState.setPathLength(pathLength);
        currentState.setEnergy(batLev);
        currentState.start(this, panel);// TODO this is where the controller calls on STATE WINNING AT END OF GAME || ORIGINAL LINE
    ...
    public void switchToTitle() {
        ...
        if(this.pan != null) {
        	...
        	System.out.println("got past mypanel error");
        }
        currentState.start(this, panel);
    ...
    public boolean keyDown(UserInput key, int value) {
    	/*try {
			System.out.println("x-cord: " + robot.getCurrentPosition()[0] + " y-cord: "+ robot.getCurrentPosition()[1]);
		} catch (Exception e) {
    ...
    /**
     * this sends the maze application to an error message saying that the robot ran out of batter
     */
    /*public void batErrorScreen() {

		robot.setBatteryLevel(-1);
		this.switchFromPlayingToWinning(0, 0);
    }*/
    ...
    public void waitForPlayingState() {
    	if(currentState == states[1]) {
    		((StateGenerating)states[1]).factory.waitTillDelivered();
    	}
    }
    
    public void setMyPanel(JPanel pan) {
    	this.pan = pan;
    }
