...
public class FirstPersonDrawer {
	...
	public void draw(MazePanel panel, int x, int y, int walkStep, int ang) {
		drawingFPV(panel, x, y, walkStep, ang);
	}
	 ...
	 * @param ang
	 */
	private void drawingFPV(MazePanel panel, int x, int y, int walkStep, int ang) {
		// obtain a Graphics2D object we can draw on
		//panel.createBufferGraphics();
	...
	private void drawSegment(Seg seg) {
		...
		panel.setColor(seg.getColor()[0], seg.getColor()[1], seg.getColor()[2]);// TODO check this later if maze is still white
		//panel.setColor(192, 192, 192);
