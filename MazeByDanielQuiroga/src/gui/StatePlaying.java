...
public class StatePlaying extends DefaultState {
    ...
    public boolean keyDown(UserInput key, int value) {
        ...
        case Up: // move forward
            walk(1);
            // System.out.println("X-cord: " + this.getCurrentPosition()[0] + " Y-cord: " + this.getCurrentPosition()[1]);
            //System.out.println(this.getCurrentPosition()[1]);
            // check termination, did we leave the maze?
            if (isOutside(px,py)) {
                control.switchFromPlayingToWinning(0, 0);// TODO calls on controller to switch to final screen
            }
            break;
        case Left: // turn left
            rotate(1);
